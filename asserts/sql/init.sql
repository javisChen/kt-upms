...
-- -----------------------------------------------------
-- Table `kt`.`upms_system`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `kt`.`upms_system` ;

CREATE TABLE IF NOT EXISTS `kt`.`upms_system` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(50) NOT NULL COMMENT '系统名称',
  `code` VARCHAR(50) NOT NULL COMMENT '系统编码',
  `status` TINYINT(1) UNSIGNED NOT NULL COMMENT '状态 1-已上线；2-已下线；',
...
CREATE TABLE IF NOT EXISTS `kt`.`upms_route` (
  ...
  INDEX `idx_is_deleted` (`is_deleted` ASC),
  INDEX `idx_pid` (`pid` ASC),
  INDEX `fk_idx_system_id` (`system_id` ASC),
  INDEX `idx_system_id` (`is_deleted` ASC, `system_id` ASC))
ENGINE = InnoDB
COMMENT = '路由表';
...
CREATE TABLE IF NOT EXISTS `kt`.`upms_api` (
  ...
  UNIQUE INDEX `uk_code` (`code` ASC),
  INDEX `uk_is_deleted` (`is_deleted` ASC),
  INDEX `fk_upms_api_upms_app1_idx` (`system_id` ASC),
  INDEX `uk_system_id` (`system_id` ASC, `is_deleted` ASC))
COMMENT = 'api表';
