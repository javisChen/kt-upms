...
CREATE TABLE IF NOT EXISTS `kt`.`upms_user` (
  ...
  UNIQUE INDEX `code_UNIQUE` (`code` ASC, `is_deleted` ASC))
ENGINE = InnoDB
COMMENT = '用户表';
...
-- -----------------------------------------------------
-- Table `kt`.`upms_api_category`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `kt`.`upms_api_category` ;

CREATE TABLE IF NOT EXISTS `kt`.`upms_api_category` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `application_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '应用id，关联upms_application.id',
