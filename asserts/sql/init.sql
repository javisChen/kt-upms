...
CREATE TABLE IF NOT EXISTS `kt`.`upms_user_group_user_rel` (
  ...
  `modifier` BIGINT NULL DEFAULT -1 COMMENT '更新人',
...
CREATE TABLE IF NOT EXISTS `kt`.`upms_permission` (
  ...
  `modifier` BIGINT NULL DEFAULT 0 COMMENT '更新人',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uk_type_resource_id` (`type` ASC, `resource_id` ASC),
...
CREATE TABLE IF NOT EXISTS `kt`.`upms_page_element` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'id',
  `menu_id` BIGINT UNSIGNED NOT NULL COMMENT '所属菜单id，关联upms_menu.id',
...
CREATE TABLE IF NOT EXISTS `kt`.`upms_route` (
  ...
  `level_path` VARCHAR(45) NOT NULL COMMENT '路由层级路径，例如：0.1.2 代表该菜单是三级路由，上级路由的id是1,再上级的路由id是0',
  ...
  `gmt_modified` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
  `creator` BIGINT NOT NULL DEFAULT -1 COMMENT '创建人',
  `modifier` BIGINT NOT NULL DEFAULT -1 COMMENT '更新人',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uk_name` (`name` ASC),
  UNIQUE INDEX `uk_key` (`code` ASC))
ENGINE = InnoDB
COMMENT = '路由表';
